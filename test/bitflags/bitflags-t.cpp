 ...
 *
 * This test goes through every method at least once and tests everything with a variety of parameters. If you
 * feel that the test is inadequate, feel free to update and create a pull request. Also please report any
 * bugs that may appear through more testing conditions. When built, this test is automatically run when ctest
 * is executable from the command line inside of the build directory.
 *
 * @return 0 if test passes, -1 if test fails.
 */
int main()
{
  Breeze::BitFlags<TestClass> bf1({}, static_cast<int>(TestClass::count));
  Breeze::BitFlags<TestClass, int> bf2({TestClass::b});
  Breeze::BitFlags<TestClass, long int> bf3({TestClass::a, TestClass::d});
