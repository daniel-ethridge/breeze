...
namespace Breeze
{
   ...
   *
   * This class can be used to create bitflags from any enum class whose members are all set to ascending
   * powers of 2 starting from one (eg 0x1, 0x2, 0x4, 0x8, 0x10, etc or 1 << 0, 1 << 2, 1 << 3, etc.).
   *
   ...
   * Integer type. Defaults to int which allows for 16 different state values or booleans. Other values such
   * as long int can be used instead to allow for more states.
   */
  ...
  public:
     ...
     * \param numFlags Optionally include the number of flags. This corresponds to the number of members in
     * the enum class. A nice way to have this value is to end the enum class with a member named 'COUNT'.
     */
    BitFlags(std::vector<T> flags, int numFlags = 0) : m_flags(0), m_numFlags(numFlags)
    {
      for (auto flag : flags)
     ...
     *
     * This method will turn off the currently active flags that were passed, and it will turn on the
     * currently inactive flags.
     *
     ...
     *
     * \param numFlags The total number of flags in the object. Only needs to be passed if it was not passed
     * in the constructor.
     */
     ...
     *
     * \param numFlags The total number of flags in the object. Only needs to be passed if it was not passed
     * in the constructor. \return True if all flags are turned on. Otherwise false.
     */
