...
namespace Breeze
{
	 ...
	 * @tparam IntegerType Defaults to int. Being a 4 byte type, it allows for 32 different state values or booleans. Other values such as
	...
	public:
		 ...
		 *
		 * The constructor is defined this way to help avoid accidentally forgetting to cast a COUNT member of the passed
		 * enum class to an integer. Such a mistake would be easy to make, though since it compiles fine debugging it would
		 * potentially be painful. Forcing the initial flags to be a vector (or an empty vector for all flags off) gets
		 * around this issue.
		 *
		 * @param flags The flags with an initial value of on.
		...
		{
			assert(numFlags <= CHAR_BIT * sizeof(int_type));
			for (auto flag : flags)
