...
namespace Breeze::Orm
{
	...
	void QueryBuilder::run(const QString& queryStr) { QSqlQuery query(queryStr); }
	                                      ...
	                                      const QString& connectionName)
	{
		DatabaseConnection db =
		    static_cast<DatabaseConnection>(QSqlDatabase::addDatabase("QPSQL", connectionName));
	...
	void Model::parseQuery(QSqlQuery&& query)
	{
		...
		// auto test = list[0].value<QVector<int>>();

		/*auto parse = [](QVariantList& qList)
		{
			switch (static_cast<DbColumnTypes>(qList[2].toInt())
			{
			case DbColumnTypes::INTEGER:
				auto test = qList[0].value<QVector<int>>();
				break;
		...
		std::for_each(u_attributes.begin(),
		              u_attributes.end(),
		              [](QVariantList list) {});*/
	}
